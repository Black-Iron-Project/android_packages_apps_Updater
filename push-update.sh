...
updates_dir=/data/blackiron_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have blackiron-VERSION-DATE-TYPE-* as format"
...
# Assume blackiron-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.blackiron.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.blackiron.updater/databases/updates.db" \
